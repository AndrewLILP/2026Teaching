...
### Graph / network diagram (nodes & labelled edges)
```html
<figure class="graph-diagram">
  <svg viewBox="0 0 600 400" role="img" aria-labelledby="g1-title g1-desc">
    <title id="g1-title">Short title</title>
    <desc id="g1-desc">One-sentence text equivalent of what the graph shows.</desc>
    <line class="graph-edge" x1=".." y1=".." x2=".." y2=".."/>
    <rect class="graph-edge-label-bg" x=".." y=".." width=".." height=".."/>
    <text class="graph-edge-label" x=".." y="..">Edge label</text>
    <!-- repeat lines/labels for each edge, THEN draw nodes on top -->
    <g class="graph-node graph-node--focal">
      <circle cx=".." cy=".." r="36"/>
      <text x=".." y="..">Focal node</text>
    </g>
    <g class="graph-node">
      <circle cx=".." cy=".." r="30"/>
      <text x=".." y="..">Other node</text>
    </g>
  </svg>
  <figcaption>Caption explaining what the diagram represents.</figcaption>
</figure>
```
Added building the Networking Systems and Social Computing unit's Lesson 2, for social-network-analysis node/edge diagrams — `.ipo-flow` is a linear chain and can't represent a focal node with multiple branches. Raw SVG, no JS, lives in `ct-shared.css` Layer 5 using only existing CSS variables, so it themes correctly under either the dark system or with `ct-lessons.css` layered on top. Draw all edges (lines + label backgrounds + labels) before any node `<g>` elements in document order, so nodes paint on top and edges never visually cross over one. At most one `.graph-node--focal` per diagram. Keep node labels to one or two short words — longer labels need a `<tspan>` line-break or will overflow the circle. **Not a layout engine** — coordinates are hand-placed per diagram, same philosophy as `.flowchart` being scoped to one decision point. Beyond roughly 8 nodes this hand-placed approach gets unwieldy; flag that case rather than forcing it.
...
### Category grid (infographic-style grouping)
```html
<div class="category-grid">
  <div class="category-column">
    <p class="category-column__title">Consumer</p>
    <p class="category-column__count">6 devices</p>
    <ul class="category-device-list">
      <li class="category-device">
        <span class="category-device__name">Device name</span>
        <span class="category-device__note">One-line note on its function.</span>
      </li>
    </ul>
  </div>
  <!-- repeat per category -->
...
```
Added building the Networking Systems and Social Computing unit's Lesson 5, for grouping a device list into labelled categories as part of an infographic task. Deliberately uncoloured per column — the site already uses one colour triad for tier badges (Foundation/Core/Extension) and a four-colour set for phase tags; a third colour triad for categories risked visually implying a relationship to tiers or phases that doesn't exist. Column identity comes from the header text alone.

### Case study box
```html
<div class="case-study-box">
  <p class="case-study-box__eyebrow">Case study</p>
  <h3>Case title</h3>
  <p>Scenario description.</p>
  <p class="case-study-box__question"><strong>Discuss:</strong> Question text.</p>
</div>
```
Added building the Networking Systems and Social Computing unit's Lesson 3, resolving an open question from that unit's brief. Distinct from `.example-box` (a syllabus-content example) and `.analysis-box` (a guided analysis task) — a case study needs a real-world scenario plus an attached discussion question as two visually distinct zones in one card. Uses `--color-tier-foundation` (amber) as a fixed identity colour regardless of the module's own accent, same design choice already made for `.analysis-box`'s fixed teal, so case studies read consistently across every module.
